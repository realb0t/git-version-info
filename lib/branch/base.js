      ...
      Promise.all([ targetHistoryReader, otherHistoryReader ])
        .then(([ targetBranchCommits, otherBranchCommits ]) =>
        {
          this.lookupJointCommit(targetBranchCommits, otherBranchCommits)
            .then(searchCommitResolve).catch(searchCommitReject);
        });
    });
  }

  lookupJointCommit(targetBranchCommits, otherBranchCommits)
  {
    return new Promise((resolve, reject) =>
    {
      ...
      {
        reject(new Error('Not found joint commit'));
      }

      const lastJointSha = firstJointShaFromTheEnd;
      resolve(lastJointSha);
    });
  }
